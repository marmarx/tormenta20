<script setup>
const props = defineProps({modelValue:Boolean,option:String})
...
<template>
  <div class="input">
    ...
    <label :for="`checkbox-${option}`">
      {{ option }}
      <input type="checkbox" :id="`checkbox-${option}`" :value="option" :checked="modelValue" @change="$emit('update:modelValue',$event.target.checked)" @click.stop/>
...
input{opacity:0;width:100%;height:100%;padding:0 1em}
