<script setup>
const props = defineProps({checked:Boolean,option:String})
</script>
...
<template>
  <div class="fakeoption">
    ...
    <label :for="`checkbox-${option}`">
      {{ option }}
      <input type="checkbox" :id="`checkbox-${option}`" :value="option" :checked="checked" @click.stop.prevent/>
...
<style scoped>
.fakeoption{display:flex;width:100%;padding:0 .8em}
